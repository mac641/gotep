...
// Headers
HEADERFIELD:
	FIELDNAME {ignoreWs = true} WHITESPACE* {ignoreWs = false} FIELDVALUE {ignoreWs = true} WHITESPACE* {ignoreWs = false } NEWLINE?
	;
...
MESSAGES : (MESSAGE_ENTRY | MESSAGELINE) NEWLINE | MULTIPARTFORMDATA;
...
fragment FILEPATH: (
	...
	; // [\r\n \t\f/]
fragment INPUTFILEREF : '<' WHITESPACE FILEPATH;
// Response handler
fragment HANDLERSCRIPT : ID+ (NEWLINE | NEWLINEWITHINDENT);
fragment RESPONSEHANDLER:
	'>' WHITESPACE '{%' HANDLERSCRIPT '%}'
	| '>' WHITESPACE FILEPATH
	; // TODO: prompt error message if used anyways

// Response reference
fragment RESPONSEREF : '<>' WHITESPACE FILEPATH;

// Messages
fragment MULTIPARTFORMDATA	: MULTIPARTFIELD MULTIPARTFORMDATA? BOUNDARY;
fragment MULTIPARTFIELD		: BOUNDARY (HEADERFIELD NEWLINE)* NEWLINE MESSAGES?;
